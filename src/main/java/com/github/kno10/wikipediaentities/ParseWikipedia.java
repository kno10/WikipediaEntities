...
public class ParseWikipedia {
	 ...
	 */
	public synchronized Thread makeReaderThread(String fname,
			BlockingQueue<Article> queue) {
		++readers;
		return new ReaderThread(fname, queue);
	}
	...
	private class ReaderThread extends Thread {
		 ...
		 *            Source file name
		 * @param queue
		 *            Queue to output articles to
		 */
		public ReaderThread(String fname, BlockingQueue<Article> queue) {
			this.fname = fname;
			this.queue = queue;
		}
		...
		private void parsePage(XMLEventReader eventReader)
			...
			if (skip || title == null || text == null)
				return;
			queue.put(new Article(title, redirect, text));
	...
	private class WikipediaParserThread extends Thread {
		...
		private Matcher redirmatcher = Pattern
				.compile(
		...
		private void process(Article a) {
			...
			if (title.startsWith("List of ") || title.startsWith("Liste von "))
				return;
	...
	public static void main(String[] args) {
		...
		try {
			...
			// Start the reader:
			Thread reader = l.makeReaderThread(Config.get("loader.source"), q1);
			RedirectCollector r = new RedirectCollector(
					Config.get("redirects.output"));
			LuceneWikipediaIndexer indexer = new LuceneWikipediaIndexer(
					Config.get("indexer.dir"));
